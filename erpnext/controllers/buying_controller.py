...
class BuyingController(StockController):
	...
	def validate_accepted_rejected_qty(self):
		for d in self.get("items"):
			self.validate_negative_quantity(d, ["received_qty","qty", "remained_qty"])
			if not flt(d.received_qty) and flt(d.qty):
				d.received_qty = flt(d.qty) - flt(d.remained_qty)

			elif not flt(d.qty) and flt(d.remained_qty):
				d.qty = flt(d.received_qty) - flt(d.remained_qty)

			elif not flt(d.remained_qty):
				d.remained_qty = flt(d.received_qty) -  flt(d.qty)

			# Check Received Qty = Accepted Qty + Rejected Qty
			if ((flt(d.qty) + flt(d.remained_qty)) != flt(d.received_qty)):
				frappe.throw(_("Accepted + remained_qty Qty must be equal to Received quantity for Item {0}").format(d.item_code))

			# Check Delevired Qty >= Received Qty
			if ((flt(d.received_qty) < flt(d.qty))):
				frappe.throw(_("Delivered Qty must be larger than or equal to Received quantity for Item {0}").format(d.item_code))
