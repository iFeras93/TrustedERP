...
/*
erpnext.item.ItemController = erpnext.TransactionController.extend({
	...
	item_code: function(doc, cdt, cdn){
		...
		get_item_material_detail(doc, cdt, cdn, scrap_items);
	},
})

$.extend(cur_frm.cscript, new erpnext.item.ItemController({frm: cur_frm}));

var get_item_material_detail= function(doc, cdt, cdn, scrap_items) {
	var d = locals[cdt][cdn];
	if (d.item_code) {
		return frappe.call({
			doc: doc,
			method: "get_item_material_detail",
			args: {
...
// rm : raw material
erpnext.item.calculate_rm_cost = function(doc) {
	var rm = doc.items || [];
	total_rm_cost = 0;
...
//sm : scrap material
erpnext.item.calculate_scrap_materials_cost = function(doc) {
